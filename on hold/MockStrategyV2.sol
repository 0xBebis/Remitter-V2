...
contract ERC20 is Context, IERC20 {
     ...
     *
     * This public function is equivalent to `approve`, and can be used to
 ...
 */
contract MockReaperAutoCompoundV2 is Ownable, Pausable {
    ...
    address public defaultRewardToken = address(0x05848B832E872d9eDd84AC5718D58f21fD9c9649);
    address public defaultLpPair;

    //lpToken => lpTokrn0
    address public defaultLpToken0;
    address public defaultLpToken1;
     ...
     */
    address public defaultRouter = address(0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52);
    address public defaultMasterChef = address(0x5bC37CAAA3b490b65F5A50E2553f4312126A8b7e);
    uint8 public poolId;
     ...
     */
    address[] public rewardTokenToWftmRoute = [defaultRewardToken, wftm];
    ...
    constructor (
      ...
      address _treasury
        ...
        defaultLpPair = _lpPair;
        poolId = _poolId;
        vault = _vault;
        treasury = _treasury;

        _mapRoutes(_lpPair, defaultRewardToken);

        _initializeBaseAllowances();
    }

    mapping (address => address[]) public rewardToToken0Routes;
    mapping (address => address[]) public rewardToToken1Routes;

    function _mapRoutes(address _lpPair, address _reward) public returns (bool) {
      address lpToken0 = IUniswapV2Pair(_lpPair).token0();
      address lpToken1 = IUniswapV2Pair(_lpPair).token1();

      if(lpToken0 == wftm){
        rewardToToken0Routes[_lpPair] = [_reward, wftm];
      }else{
        rewardToToken1Routes[_lpPair] = [_reward, wftm, lpToken0];
      }
      if(lpToken1 == wftm){
        rewardToToken0Routes[_lpPair] = [_reward, wftm];
      }else{
        rewardToToken1Routes[_lpPair] = [_reward, wftm, lpToken1];
      }
      return true;
    }
     ...
     * It deposits {defaultLpPair} in the masterChef to farm {}
     */
    function deposit() public whenNotPaused {
      uint256 pairBal = IERC20(defaultLpPair).balanceOf(address(this));

      if (pairBal > 0) {
        IMasterChef(defaultMasterChef).deposit(poolId, pairBal);
      }
    }

    /**
     * @dev Withdraws funds and sents them back to the vault.
     * It withdraws {defaultLpPair} from the masterChef.
     * The available {defaultLpPair} minus fees is returned to the vault.
     */
    function withdraw(uint256 _amount) external {
      ...
      if (pairBal < _amount) {
        IMasterChef(defaultMasterChef).withdraw(poolId, _amount.sub(pairBal));
        pairBal = IERC20(defaultLpPair).balanceOf(address(this));
      }
      ...
      IERC20(defaultLpPair).safeTransfer(vault, pairBal.sub(withdrawFee));
    }
     ...
     */
    function _harvest() external whenNotPaused {
      require(!Address.isContract(msg.sender), "!contract");
      IMasterChef(defaultMasterChef).deposit(poolId, 0);
      _chargeFees(defaultRewardToken, defaultRouter);
      _addLiquidity(defaultLpPair, defaultRewardToken, defaultRouter);
      deposit();

      emit StratHarvest(msg.sender);
    }

    mapping(address => address[]) public rewardToWFTMRoute;

    function _mapRewardToWFTMRoute(address _reward) public returns (bool) {
      rewardToWFTMRoute[_reward] = [_reward, wftm];
      return true;
    }
     ...
     */
    function _chargeFees(address _reward, address _router) public {

      if (rewardToWFTMRoute[_reward].length == 0) {
        _mapRewardToWFTMRoute(_reward);
      }

      uint256 toWftm = IERC20(_reward).balanceOf(address(this)).mul(totalFee).div(PERCENT_DIVISOR);
      IUniswapRouterETH(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWftm, 0, rewardToWFTMRoute[_reward], address(this), now.add(600));
     ...
     */
    function _addLiquidity(address _lpPair, address _reward, address _router) public {

      address lpToken0 = IUniswapV2Pair(_lpPair).token0();
      address lpToken1 = IUniswapV2Pair(_lpPair).token1();

      if (rewardToToken0Routes[_lpPair].length == 0) {
        _mapRoutes(_lpPair, _reward);
      }

      uint256 half = IERC20(_reward).balanceOf(address(this)).div(2);

      if (lpToken0 != _reward) {
        IUniswapRouterETH(_router)
          .swapExactTokensForTokensSupportingFeeOnTransferTokens(
            half, 0, rewardToToken0Routes[_lpPair], address(this), now.add(600)
          );
      } if (lpToken1 != _reward) {
        IUniswapRouterETH(_router)
          .swapExactTokensForTokensSupportingFeeOnTransferTokens(
            half, 0, rewardToToken1Routes[_lpPair], address(this), now.add(600)
          );
      }
      uint256 lp0Bal = IERC20(lpToken0).balanceOf(address(this));
      uint256 lp1Bal = IERC20(lpToken1).balanceOf(address(this));
      IUniswapRouterETH(_router).addLiquidity(lpToken0, lpToken1, lp0Bal, lp1Bal, 1, 1, address(this), now.add(600));
    }

    function sweepExtraRewards(address lpPair, address rewardToken, address router) external returns (bool){
      address lpToken0 = IUniswapV2Pair(lpPair).token0();
      address lpToken1 = IUniswapV2Pair(lpPair).token1();
      _giveAllowances(router, rewardToken, lpPair, lpToken0, lpToken1, defaultMasterChef);
      _mapRoutes(lpPair, rewardToken);
      _chargeFees(rewardToken, router);
      _addLiquidity(lpPair, rewardToken, router);
      return true;
    }
    ...
    function balanceOfLpPair() public view returns (uint256) {
      return IERC20(defaultLpPair).balanceOf(address(this));
    }
    ...
    function balanceOfPool() public view returns (uint256) {
      (,uint256 _amount) = IMasterChef(defaultMasterChef).getUserInfo(poolId, address(this));
      return _amount;
    }
    ...
    function retireStrat() external {
      ...
      IMasterChef(defaultMasterChef).emergencyWithdraw(poolId);
      uint256 pairBal = IERC20(defaultLpPair).balanceOf(address(this));
      IERC20(defaultLpPair).transfer(vault, pairBal);
    }
    ...
    function panic() public onlyOwner {
        pause();
        IMasterChef(defaultMasterChef).withdraw(poolId, balanceOfPool());
    }
    ...
    function _initializeBaseAllowances() public returns (bool) {
      _giveAllowances(
        defaultRouter,
        defaultRewardToken,
        defaultLpPair,
    ...
    function _revokeBaseAllowances() public returns (bool) {
      _removeAllowances(
        defaultRouter,
        defaultRewardToken,
        defaultLpPair,
    ...
    function _giveAllowances(
      ...
      address _chef
        ...
        IERC20(_lpPair).safeApprove(_chef, uint256(-1));
        IERC20(_reward).safeApprove(_router, uint256(-1));

        IERC20(_lpToken0).safeApprove(_router, 0);
        IERC20(_lpToken0).safeApprove(_router, uint256(-1));

        IERC20(_lpToken1).safeApprove(_router, 0);
        IERC20(_lpToken1).safeApprove(_router, uint256(-1));
    }
    ...
    function _removeAllowances(
      ...
      address _chef
        ...
        IERC20(_lpPair).safeApprove(_chef, 0);
        IERC20(_reward).safeApprove(_router, 0);
        IERC20(_lpToken0).safeApprove(_router, 0);
        IERC20(_lpToken1).safeApprove(_router, 0);
    }
